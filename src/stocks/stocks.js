...
const Stocks = () => {
  const URL = "https://cloud.iexapis.com/stable/stock/market/batch?symbols=aapl,tsla,f,ge,msft,pfe,aalnio,dis,amzn&types=quote,chart&range=1d&last=5&token=pk_6f789411fea3492293da22e99ff8d631";
  ...
  const getStocks = () => {
    return axios({
      ...
      setDisplayData(Object.keys(resp).slice(index,item).map(key => ({ [key]: resp[key] })));
      return resp;
    })
  };
  const showData = () => {
    return displayData.map(data => {
      return Object.entries(data).map(([key, value]) => {
        ...
        return <div className="stocks" key={value.quote.symbol}>
          ...
          <div className="flex-2">
            <div className="chart">
              ...
              <div className="flex-1 button">
                <button type="button" className="btn">{value.quote.latestPrice}</button>
              </div>
            </div>
          </div>
        </div>;
  ...
  const togglePrev = (e) => {
    ...
    setDisplayData(Object.keys(stocksData).slice(startIndex, endindex).map(key => ({ [key]: stocksData[key] })));
    setIndex(startIndex);
  ...
  const toggleNext = (e) => {
    let startIndex = index + item;
    let endIndex = startIndex+item;
    if(startIndex>=stocksData.length)
        startIndex=stocksData.length;
    let disabledNext = endIndex <= (Object.keys(stocksData).length - 1);
    ...
    <div className="stocks-card"
      ...
      <div className="flex-column">
        ...
        <div className="flex-1 right-slider">
          {showButton && disabledNext ? <button type="submit" className="right"
